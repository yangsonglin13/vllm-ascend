...
5. Upgrade the compatibility protocol to RFork v2:
   - canonical compatibility fingerprint;
   - collision-free seed keys;
...
6. Centralize RFork environment variables in `vllm_ascend/envs.py` with
   `VLLM_ASCEND_RFORK_*` names while retaining documented legacy fallbacks.
7. Add shared-token authentication hooks and configurable bind/advertise
...
8. Add unit coverage for the loader, transfer backend, worker, seed protocol,
...
`model_loader_extra_config` remains the primary configuration source. Canonical
environment fallbacks are defined only in `vllm_ascend/envs.py`:

- `VLLM_ASCEND_RFORK_MODEL_URL`
...
RFork v2 hashes a canonical JSON descriptor containing model identity,
deployment strategy, dtype, quantization, revision, model architecture,
parallel world sizes, worker role/ranks, KV role, Ascend weight layout mode,
draft role, and protocol version. The planner sees an opaque versioned seed key,
so separators inside user-controlled values cannot create collisions.
