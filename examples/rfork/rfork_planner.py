...
class Settings:
    ...
    def from_env() -> Settings:
        return Settings(
            ...
            lease_ttl_sec=int(os.getenv("RFORK_MOCK_LEASE_TTL_SEC", "300")),
            ...
            auth_token=os.getenv("RFORK_MOCK_AUTH_TOKEN") or None,
        )
...
class SeedRecord:
    ...
    resource_total: int
    resource_used: int = 0
    # Allocation recency is separate from heartbeat recency.  Heartbeats
    # describe liveness and must not make an otherwise idle seed look recently
    # used to an LRU allocator.
...
class Scheduler:
    ...
    def choose_seed(self, seeds: Iterable[SeedRecord]) -> SeedRecord | None:
        ...
        # LRU allocates the least recently allocated seed. Heartbeats update
        # liveness only and must not change this ordering.
...
def build_router(store: Store, auth_token: str | None = None):
    router = APIRouter()
    ...
    def remove_seed(request: Request) -> Response:
        ...
        # Idempotent cleanup: an already GC'd seed is a successful desired
        # state for shutdown callers.
        if not removed:
    ...
    def debug_snapshot(request: Request) -> Response:
        if not authorized(request):
            return Response(status_code=status.HTTP_401_UNAUTHORIZED)
...
def create_app(settings: Settings):
    ...
    app.include_router(build_router(store, settings.auth_token))
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="independent lease ttl in seconds (default: env RFORK_MOCK_LEASE_TTL_SEC or 300)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--auth-token",
        default=defaults.auth_token,
        help="optional shared RFork auth token (default: env RFORK_MOCK_AUTH_TOKEN)",
    )
    return parser
