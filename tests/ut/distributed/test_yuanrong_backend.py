...
def _make_full_backend(tmp_path, monkeypatch, lazy_init=False, **overrides):
    ...
    backend_module = sys.modules[YuanrongBackend.__module__]
    monkeypatch.setattr(backend_module, "split_host_port", lambda _: ("127.0.0.1", 31501))
    return YuanrongBackend(MagicMock(), lazy_init=lazy_init)


def test_backend_accepts_lazy_init_and_initializes_eagerly(tmp_path, monkeypatch, caplog):
    backend = _make_full_backend(tmp_path, monkeypatch, lazy_init=True)

    backend.store.init.assert_called_once_with()
    assert "does not support lazy initialization; initializing eagerly" in caplog.text
