...
def test_binds_nested_lm_head_as_lm_head(load_state):
    target_head = SimpleNamespace(weight=object())
    nested_head = SimpleNamespace(weight=object())
    root_head = SimpleNamespace(weight=object())
    draft = SimpleNamespace(model=SimpleNamespace(lm_head=nested_head), lm_head=root_head)
    ...
    assert draft.model.lm_head is target_head
    assert draft.lm_head is target_head
