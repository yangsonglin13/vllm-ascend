...
def test_real_loader_rebuilds_mla_derived_tensors_after_checkpoint_transfer(request, monkeypatch):
    ...
    loader.session.transfer_from_seed.side_effect = lambda model, processed: backend.read_weights_from_seed(
        model, info, processed
    )
    ...
    torch.testing.assert_close(destination.impl.mlapo_W_UK_T, source.impl.mlapo_W_UK_T)
