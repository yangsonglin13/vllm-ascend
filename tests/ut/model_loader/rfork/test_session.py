...
def _make_session(monkeypatch, **config_overrides):
    ...
    identity = RForkIdentity("kv_both", 0, 0, 0, compatibility_fingerprint="fp")
...
def test_constructor_passes_typed_config_and_identity(monkeypatch):
    ...
    assert session.transfer_backend.kwargs == {"request_timeout_sec": 1.5, "auth_token": "secret"}
...
def test_acquire_transfer_release_updates_state(monkeypatch):
    ...
    assert session.transfer_from_seed(object(), False)
