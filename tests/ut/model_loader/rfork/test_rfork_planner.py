...
from examples.rfork.rfork_planner import Scheduler, Settings, Store, create_app
...
def test_planner_auth_rejects_missing_token_and_accepts_shared_token():
    test_client = pytest.importorskip("fastapi.testclient").TestClient
    app = create_app(Settings(auth_token="secret", heartbeat_sweep_sec=1))
