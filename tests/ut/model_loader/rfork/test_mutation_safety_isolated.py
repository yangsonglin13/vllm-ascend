...
def test_worker_weight_updates_without_session_reach_engine(worker_methods, operation):
    engine = Mock()
    ...
    args = ({"weights": "chunk"},) if operation == "update_weights" else ()
    worker_methods[operation](worker, *args)
    getattr(engine, operation).assert_called_once_with(*args)
