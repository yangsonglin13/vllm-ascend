...
@pytest.mark.parametrize(
    ("remote_only_name", "expected_success"),
    [("layers.0.attn.impl.W_UV", True), ("layers.0.attn.impl.unexpected_cache", False)],
)
def test_checkpoint_transfer_only_ignores_known_post_load_tensors(remote_only_name, expected_success):
    ...
    backend.excluded_weight_blocks = []
    backend._registered_transferable_tensors = [("weight", weight)]
    entry = [1, weight.numel(), weight.element_size(), list(weight.shape), "float32"]
    seed_info = SeedTransferInfo(
        "seed-session",
        {"weight": entry, remote_only_name: entry},
        formats={"weight": 0, remote_only_name: 0},
    )

    assert backend.read_weights_from_seed(object(), seed_info, False) is expected_success
    assert bool(reads) is expected_success
