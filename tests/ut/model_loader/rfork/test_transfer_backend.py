...
def test_read_weights_from_seed_fails_for_unknown_weight_outside_shared_blocks(monkeypatch):
    ...
    seed_info = SeedTransferInfo("seed-session", {})
...
def test_incompatible_manifest_rejected_before_native_read(monkeypatch, entry):
    ...
    backend._registered_transferable_tensors = [("weight", tensor)]
    seed_info = SeedTransferInfo("session", {"weight": entry})
...
@pytest.mark.parametrize(
    ("formats", "expected_success"),
    [
        ({"layers.0.fc.weight": 3}, False),
        ({"layers.0.fc.weight": 2}, True),
        ({"layers.0.other.weight": 2}, False),
    ],
    ids=["divergent", "matching", "malformed-name"],
)
def test_read_weights_from_seed_validates_storage_formats(monkeypatch, formats, expected_success):
    ...
    monkeypatch.setattr(rfork_manifest, "read_npu_format", lambda tensor: 2)
