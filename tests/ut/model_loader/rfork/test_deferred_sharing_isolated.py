...
def test_seed_transfer_info_carries_shared_names_and_load_state(monkeypatch):
    runtime = _make_runtime(monkeypatch)
    session = _make_session(runtime)
    session.transfer_backend.transfer_session_id = "session"
    session.transfer_backend.weight_manifest = {"weight": (1, 1, 4, (1,), "float32")}
    ...
    info = session._seed_transfer_info({"has_own_lm_head": False})

    assert info.shared_names == ("model.embed_tokens.weight",)
    assert info.formats == {"weight": 2}
