    ...
    *,
    ...
    try:
        ...
        if startup_state is not None:
            # Publish the resource before queue.put and serialize it with the
            # caller's timeout cleanup.  A queue timeout must never make a
            # just-created socket unreachable from the cleanup path.
...
def _stop_startup_thread(
    ...
    # Set the event and take a snapshot of the published startup resource in
    # one critical section.  The worker publishes before queue.put and checks
    # the event under the same lock, closing the pre-handoff race.
    with startup_state.lock:
        stop_event.set()
