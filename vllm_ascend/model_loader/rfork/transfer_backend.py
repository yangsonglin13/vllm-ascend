...
def _tensor_storage_owner(tensor: torch.Tensor) -> Any:
    """Return an object that keeps the tensor's original storage alive.

    Post-load processing may replace a tensor's storage after registration.
    Holding only the Tensor object would then keep the new storage alive while
    the memory registration still points at the old one.  Keep the storage
    object itself for the lifetime of the registration.
    """
...
class RForkTransferBackend:
        ...
        if logical_registrations:
            ...
            for batch_start in range(0, len(logical_registrations), MAX_MEMORY_REGISTRATION_BATCH_ITEMS):
                ...
                if result is None or result.is_error():
                    ...
                    # An error does not identify which registrations remain.
                    # Keep every attempted address and its storage owner until
                    # unregistration confirms that the region is absent.
    ...
    def _unregister_weight_blocks(self, transfer_engine: Any) -> bool:
        ...
        if configured_addresses:
            remaining_addresses = list(dict.fromkeys(configured_addresses))
        else:
            remaining_addresses = list(
                dict.fromkeys(address for address, _ in getattr(self, "registered_weight_blocks", []))
            )
        ...
        while remaining_addresses:
            ...
            if result is None or result.is_error():
                if result is not None and self._is_not_found_result(result):
                    # Batch unregister can stop at its first missing address,
                    # leaving later addresses untouched.  Retry each address
                    # independently so an already-removed region is harmless
                    # and every still-registered region gets a chance to be
                    # released.
    ...
    def _is_not_found_result(self, result: Any) -> bool:
        ...
        expected_code = getattr(self, "_not_found_error_code", None)
        return expected_code is not None and code == expected_code
    ...
    def _read_weights_from_seed_locked(
        ...
        if not transferable_tensors:
            if (
                ...
                and getattr(self, "weight_manifest", None) == {}
        ...
        # Keep reads per logical name after manifest validation. Aliases may
        # reread overlapping bytes; accept that cost for now. Merging by local
        # address alone is unsafe unless the remote-to-local offset mapping is
        # also identical throughout the overlap.
        for name, tensor in transferable_tensors:
            if name in skipped_shared_names:
                continue
