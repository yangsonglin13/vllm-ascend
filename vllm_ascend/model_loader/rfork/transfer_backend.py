...
class RForkTransferBackend:
    ...
    def __init__(self, request_timeout_sec: float = DEFAULT_REQUEST_TIMEOUT_SEC, auth_token: str = "") -> None:
    ...
    def register_memory_region(self, model, processed_layout: bool) -> bool:
        with self._lock():
            if self.registered_weight_blocks:
                logger.error("RFork memory is already registered.")
                return False
    ...
    def _recv_from_source_locked(
        ...
        if local_name_set != remote_name_set:
            logger.error(
        ...
        requires_v2 = local_seed_key.startswith("rfork-v2:")
        ...
        for name, raw_info in seed_info.weights.items():
            ...
            if shape is not None and numel_from_shape(shape) != numel:
                ...
                return False
            if requires_v2 and (shape is None or dtype is None):
                logger.error("RFork v2 metadata for %s requires shape and dtype", name)
                return False
