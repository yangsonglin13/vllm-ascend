...
def _parse_weight_info(weight_info: Any):
    """Parse an RFork manifest entry.

    The minimal wire format is ``[ptr, numel, element_size]`` with an optional
    fourth shape item.  A fifth dtype item, or a JSON object with named
    fields, marks a full-metadata entry and returns
    ``(ptr, numel, element_size, shape, dtype)``; shorter entries omit the
    trailing fields.
    """
...
def _update_registered_weight_info(
    ...
    """Keep the inline manifest synchronized after metadata-only reshape."""
...
def _unpack_weight_info(parsed_weight_info: tuple[Any, ...]) -> tuple[int, int, int, Any, str | None]:
    """Normalize four-item and five-item parser results."""
...
def _extract_manifest_entries(manifest_metadata: Any) -> Mapping[str, Any] | None:
    """Extract a per-tensor metadata mapping from manifest envelopes."""
...
class RForkTransferBackend:
    ...
    def register_memory_region(
        ...
        if not merged_blocks and transferable_tensors:
            logger.error("RFork found no allocator blocks for %d transferable tensors", len(transferable_tensors))
            return False

        # Cut target-owned ranges out of the blocks to register; HCCL rejects
        # overlapping registrations of the same memory.
        merged_blocks = _subtract_weight_blocks(merged_blocks, excluded_blocks)

        if merged_blocks:
            addresses, sizes = zip(*merged_blocks)
            try:
                ret = transfer_engine.batch_register_memory(addresses, sizes)
            except Exception as e:
                logger.error(
                    "batch_register_memory raised for %d blocks: %s",
                    len(merged_blocks),
                    e,
                )
                return False
    ...
    def _unregister_weight_blocks(self, transfer_engine: Any) -> bool:
        ...
        try:
            ret = transfer_engine.batch_unregister_memory([address for address, _ in registered_blocks])
        except Exception as e:
    ...
    def recv_from_source(
        ...
        if request_timeout_sec == DEFAULT_REQUEST_TIMEOUT_SEC:
            # Keep the historical one-argument call shape for lightweight
        ...
        if local_only:
            # Handle legacy callers that rebuild an unfiltered tensor list.
            for name, tensor in transferable_tensors:
        ...
        if remote_only:
            ...
            full_model_tensors = dict(_iter_transferable_tensors(model, processed_layout))
        ...
        # Every manifest entry must carry both shape and dtype; manifests
        # without full metadata are not accepted.
