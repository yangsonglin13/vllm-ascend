...
def _schedule_rfork_seed_start(
    session: RForkSession,
    model: Module,
    processed_layout: bool,
    exclude_blocks: list[tuple[int, int]],
    *,
    ...
    """Defer draft seed publication until target sharing finishes."""
    try:
        result = session.schedule_deferred_seed_start(model, processed_layout, exclude_blocks)
    except Exception:
        logger.exception(
            "RFork %s model loaded from %s, but deferred seed scheduling raised; inference can continue.",
            _rfork_model_kind(session),
            load_source,
        )
        return
    if result is RForkSeedServiceStartResult.FAILED:
        logger.warning(
            "RFork %s model loaded from %s is ready, but deferred seed scheduling failed; inference can continue.",
            _rfork_model_kind(session),
            load_source,
        )
...
def _publish_rfork_seed(
    ...
    *,
    ...
    if session.identity.is_draft_model:
        _schedule_rfork_seed_start(
            session,
            model,
            processed_layout,
            exclude_blocks,
...
def _refresh_rfork_runtime_state(model: Module) -> None:
    """Refresh quant schemes' host caches without rerunning layout conversion."""
    for module in model.modules():
        # The scheme owns its derived host state; ask it to refresh instead of reaching into internals.
        quant_method = getattr(module, "quant_method", None)
        refresh = getattr(quant_method, "refresh_runtime_state_after_loading", None)
        if callable(refresh):
            refresh(module)
    ...
    def load_model(
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            try:
                ...
                if processed_layout_transfer:
                    _refresh_rfork_runtime_state(model)
                else:
