...
def _remove_discarded_compilation_hooks(
    ...
    for hook_id, hook in list(hooks.items()):
        ...
        if isinstance(handle, RemovableHandle) and handle.id == hook_id and handle.hooks_dict_ref() is hooks:
            ...
            removed_count += 1
    if removed_count:
        logger.debug("RFork fallback removed %d discarded model compilation hooks.", removed_count)
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            try:
                ...
                if exclude_blocks and session.can_reuse_shared_weights(
                    ...
                    logger.debug("RFork draft reuses all weights from the loaded target model.")
                    return model.eval()
            ...
            if session is not None:
                for _ in range(FALLBACK_CLEANUP_MAX_ATTEMPTS):
                    ...
                    if not isinstance(cleanup_result, RForkFallbackCleanupResult) or cleanup_result.can_schedule_seed:
                        break
