...
def _bind_shared_head_name(draft_model: Any, target_language_model: Any, name: str, parts: list[str]) -> None:
    """Bind one seed-declared ``shared_head.head`` to the target head."""
...
def _bind_shared_lm_head_name(draft_model: Any, target_language_model: Any, name: str, parts: list[str]) -> None:
    """Bind a top-level or nested ``lm_head`` to the target head."""
    target_lm_head = _resolve_target_lm_head(target_language_model)
    if target_lm_head is None:
        raise RuntimeError(f"RFork seed shared head weight {name!r} but the target model has no accessible lm_head.")

    parent = _navigate_module(draft_model, parts[:-2])
    if parent is None or getattr(parent, "lm_head", None) is None:
        # Target wrappers may prefix the checkpoint name while the draft keeps
        # its language model at the root.
        parent = draft_model
    old_head = getattr(parent, "lm_head", None) if parent is not None else None
    if old_head is None:
        raise RuntimeError(f"RFork seed shared {name!r} but the draft has no lm_head at that path.")
    parent.lm_head = target_lm_head
    # Some wrappers expose a root alias to the nested head as well.
    if getattr(draft_model, "lm_head", None) is old_head:
        draft_model.lm_head = target_lm_head
    ...
    for name in names:
        ...
        if name.endswith(HEAD_WEIGHT_SUFFIXES):
            if name.endswith("shared_head.head.weight"):
                ...
                _bind_shared_lm_head_name(draft_model, target_language_model, name, parts)
