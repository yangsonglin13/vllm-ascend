...
- **Processed NZ payload length:** RFork intentionally reads exactly
  ...
  widen a read.
- **Checkpoint-only regenerated tensors:** A checkpoint-layout seed may contain
  known MLA/SFA tensors generated by post-load processing, such as `W_UV`,
  `W_UK_T`, or decomposed attention weights, that the receiver has not yet
  materialized. RFork deliberately ignores only these recognized seed-only names
  and lets the receiver rebuild them in its post-load hook. Unknown names and
  receiver-only names remain incompatible. Processed-layout transfers require
  matching materialized names (apart from explicit target-sharing rules), while
  the compatibility key isolates P/D roles whose MLA/SFA construction differs.
  This asymmetry preserves the checkpoint-layout lifecycle; it is not relaxed
  name validation for processed layouts.
...
- Validate transfer accuracy and the logical-payload contract for newly supported
  NPU formats on the intended NPU and model combination; CPU tests cannot
  establish NPU storage correctness. This qualification is required when adding
  a format, but it is not a runtime requirement to copy allocator or descriptor
  padding outside a tensor's declared payload.
